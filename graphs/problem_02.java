    ...
    public int BrojPoddrva() {
        ...
        for (T vertex : adjacencyList.keySet()) {
            if (!visited.contains(vertex)) {
                visited.add(vertex);
                zbir += 1;
                BrojPoddrvaUtil(vertex, visited);
            }
        }
        return zbir;
    }

    private void BrojPoddrvaUtil(T vertex, Set<T> visited) {
        visited.add(vertex);
        for (T neighbor : getNeighbors(vertex)) {
            if (!visited.contains(neighbor)) {
                BrojPoddrvaUtil(neighbor, visited);
            }
        }
    }
}
public class chillinnn {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < n; i++) {
            graf.addVertex(sc.next());
        }
        int m = sc.nextInt();
        for (int i = 0; i < m; i++) {
            graf.addEdge(sc.next(), sc.next());
        }
        System.out.println(graf.BrojPoddrva());
    }
}
