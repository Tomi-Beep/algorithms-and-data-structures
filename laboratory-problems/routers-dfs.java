...
class AdjacencyListGraph<T> {
    ...
    public int brNaGrupi() {
        int counter = 0;
        Set<T> visited = new HashSet<>();
        for (T jazol : adjacencyList.keySet()) {
            if (!visited.contains(jazol)) {
                counter++;
                brNaGrupiUtil(jazol, visited);
            }
        }
        return counter;
    }

    public void brNaGrupiUtil(T jazol, Set<T> visited) {
        visited.add(jazol);
        for (T neighbor : getNeighbors(jazol)) {
            if (!visited.contains(neighbor)) {
                brNaGrupiUtil(neighbor, visited);
            }
        }
    }
}
...
public class Main {
    public static void main(String[] args) {
        ...
        graf.removeVertex(sc.nextInt());
        System.out.println(graf.brNaGrupi());
    }
}
