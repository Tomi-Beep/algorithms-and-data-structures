...
class BinarySearchTree<E extends Comparable<E>> {
    ...
    @SuppressWarnings({"raw", "unchecked"})
    private int dlabochina(Comparable broj, BNode<E> jazol) {
        if (jazol == null) {
            return 0;
        } else {
            if (broj.compareTo(jazol.info) < 0) {
                return 1 + dlabochina(broj, jazol.left);
            } else if (broj.compareTo(jazol.info) > 0) {
                return 1 + dlabochina(broj, jazol.right);
            } else
                return 1;
        }
    }

    public int dlabochina(int broj) {
        return dlabochina(broj, getRoot());
    }
}
...
public class chillinnn {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < n + q; i++) {
            ...
            if (naredba.equals("insert")) {
                int brojche = sc.nextInt();
                BNode<Integer> jazol = new BNode<>(brojche);
                bst.insert(brojche);
                map.put(jazol.info, jazol);
            } else {
                System.out.println(bst.dlabochina(sc.nextInt()));
            }
        }
    }
}
