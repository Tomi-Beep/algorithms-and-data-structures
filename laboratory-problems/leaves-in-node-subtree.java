...
public class NodeIterable {
    public static void main(String[] args) {
        ...
        int q = sc.nextInt();
        sc.nextLine();
        SLLTree<Integer> drvo = new SLLTree<>();
        Map<Integer, SLLTree.SLLNode<Integer>> nodeMap = new HashMap<>();
        for (int i = 0; i < n + q; i++) {
            ...
            if (naredba.equals("root")) {
                ...
                sc.nextLine();
                drvo.makeRoot(koren);
                nodeMap.put(koren, new SLLTree.SLLNode<>(koren));
            } else if (naredba.equals("add")) {
                int roditelVrednost = sc.nextInt();
                int deteVrednost = sc.nextInt();
                sc.nextLine();
                SLLTree.SLLNode<Integer> roditel = nodeMap.get(roditelVrednost);
                SLLTree.SLLNode<Integer> dete = (SLLTree.SLLNode<Integer>) drvo.addChild(roditel, deteVrednost);
                nodeMap.put(deteVrednost, dete);
            } else if (naredba.equals("ask")) {
                int broj = sc.nextInt();
                sc.nextLine();
                SLLTree.SLLNode<Integer> jazol = nodeMap.get(broj);
                System.out.println(countLeaves(jazol));
            }
        }
    }

    private static int countLeaves(SLLTree.SLLNode<Integer> jazol) {
        if (jazol == null) return 0;
        if (jazol.firstChild == null) return 1;
        int count = 0;
        SLLTree.SLLNode<Integer> child = jazol.firstChild;
        while (child != null) {
