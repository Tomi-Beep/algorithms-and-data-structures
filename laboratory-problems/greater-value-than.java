...
class BinarySearchTree<E extends Comparable<E>> {
    ...
    private int pogolemi(Comparable <E>broj, BNode<E> jazol) {
        if (jazol == null) {
            return 0;
        } else if (broj.compareTo(jazol.info) < 0) {
            return 1 + pogolemi(broj, jazol.left) + pogolemi(broj, jazol.right);
        } else {
            return 0 + pogolemi(broj, jazol.right);
        }
    }

    public int pogolemi(Comparable <E> broj) {
        return pogolemi(broj, getRoot());
    }
}
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < n + q; i++) {
            ...
            if (naredba.equals("insert")) {
                int broj = sc.nextInt();
                BNode<Integer> jazol = new BNode<>(broj);
                drvo.insert(broj);
                map.put(broj, jazol);
            } else {
                System.out.println(String.valueOf(drvo.pogolemi(sc.nextInt())));
            }
        }
    }
}
