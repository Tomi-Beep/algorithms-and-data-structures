...
public class chillinnn {
    ...
    public static void main(String[] args) {
        ...
        for (DLLNode<Integer> i = lista.getFirst(); i != null; i = i.succ) {
            if (i.element == M) {
                break;
            }
            pos++;
        ...
        k = k % n;
        //pos - k%n - 1???
        if (lista.find(M) == null) {
            System.out.println(lista.toString());
            return;
        }

        if (pos > k) {
            DLLNode<Integer> z = lista.find(M);
            DLLNode<Integer> t = lista.find(M);
            for (int i = 0; i < k; i++) {
                if (t != null && t.pred != null)
                    t = t.pred;
            }
            lista.delete(z);
            if (t != null)
                lista.insertBefore(M, t);
        } else if (k > pos) {
            DLLNode<Integer> z = lista.find(M);
            DLLNode<Integer> t = lista.getLast();
